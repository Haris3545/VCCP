...
// The <source> tag's url attribute is the outlet's real homepage (e.g.
// https://www.rollingstone.com), as opposed to <link>, which is a Google
// News redirect — this is what lets the UI show the outlet's real logo
// (see MediaView's OutletLogo) rather than Google's.
...
// Exported so lib/integrations/newsBackfill.js can reuse the exact same
// scraping/parsing logic against date-boxed queries (Google News RSS
// supports before:/after: as literal terms inside q) instead of duplicating
// it - the only difference for a backfill request is the query text itself.
export async function fetchQuery(query) {
  ...
  $('item')
    ...
    .each((_, el) => {
      ...
      const sourceNode = node.find('source').first();
      const sourceTag = sourceNode.text().trim();
      ...
      items.push({
        ...
        outletDomain: extractDomain(sourceNode.attr('url')),
        link,
