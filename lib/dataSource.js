...
export async function getDashboardData() {
  ...
  let headline = dashboardMock.headline;
  if (trends.source === 'live' && trends.points.length >= 2) {
    ...
    headline = dashboardMock.headline.map((stat) =>
      stat.id === 'trend-index'
        ? { ...stat, value: delta, caption: 'search interest vs. previous window', source: 'live' }
        : stat
    );
  }

  return { ...dashboardMock, headline };
}
